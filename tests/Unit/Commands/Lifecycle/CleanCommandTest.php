...
use PhpHive\Cli\Console\Commands\Lifecycle\CleanCommand;
 ...
 * Clean Command Test.
 *
 * Tests for the CleanCommand that cleans caches and build artifacts
 * across the monorepo. Verifies cache cleaning, workspace filtering,
 * and Turbo integration.
 */
final class CleanCommandTest extends TestCase
{
    ...
    {
        $this->assertSame('clean:cache', $this->command->getName());
        $this->assertContains('clean', $this->command->getAliases());
    }
    ...
    {
        $this->assertSame('Clean caches and build artifacts', $this->command->getDescription());
    }
    ...
    {
        $this->assertInstanceOf(CleanCommand::class, $this->command);
