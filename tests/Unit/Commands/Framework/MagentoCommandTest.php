...
use PhpHive\Cli\Console\Commands\Framework\MagentoCommand;
use PhpHive\Cli\Tests\TestCase;

/**
 * Magento Command Test.
 *
 * Tests for the MagentoCommand that runs Magento CLI commands in workspaces.
 * Verifies command configuration, argument handling, workspace selection,
 * and execution flow.
 */
final class MagentoCommandTest extends TestCase
{
    /**
     * The command instance under test.
     */
    private MagentoCommand $command;
    ...
    {
        parent::setUp();
        $this->command = new MagentoCommand();
    }
    ...
    {
        $this->assertSame('framework:magento', $this->command->getName());
    }
    ...
    public function test_command_has_correct_aliases(): void
    {
        ...
        $this->assertContains('magento', $aliases);
        $this->assertContains('mage', $aliases);
        $this->assertContains('bin/magento', $aliases);
    }
    ...
    {
        $this->assertSame('Run Magento CLI command in a workspace', $this->command->getDescription());
    }
    ...
    public function test_command_has_help_text(): void
    {
        ...
        $this->assertStringContainsString('magento', $help);
        $this->assertStringContainsString('Examples:', $help);
        $this->assertStringContainsString('indexer:reindex', $help);
    }
    ...
    public function test_command_argument_has_correct_description(): void
    {
        ...
        $this->assertStringContainsString('cache:flush', $description);
    }
    ...
    /**
     * Test that help text includes Magento-specific examples.
     *
     * Verifies the help text contains relevant Magento commands.
     */
    public function test_help_text_includes_magento_specific_examples(): void
    {
        $help = $this->command->getHelp();

        $this->assertStringContainsString('cache:flush', $help);
        $this->assertStringContainsString('setup:upgrade', $help);
        $this->assertStringContainsString('module:enable', $help);
    }

    /**
     * Test that command has bin/magento alias.
     *
     * Verifies the command can be called using bin/magento syntax.
     */
    public function test_command_has_bin_magento_alias(): void
    {
        $aliases = $this->command->getAliases();
